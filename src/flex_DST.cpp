/*
Determine if we're in Daylight Saving Time and adjust time accordingly.
Adapted from Andy Doro's library
https://github.com/andydoro/DST_RTC
This algorithm is programmed to observe Daylight Saving Time based on a
configuration given on the constructor. Since the rules for DST vary by country
and territory, one can define the specific rules for any place.

https://en.wikipedia.org/wiki/Daylight_saving_time_by_country
This method doesn't check whether it's 2am when the time change occurs.
...
flex_DST::flex_DST(uint8_t beginSunday, uint8_t beginMonth, uint8_t endSunday,
                   uint8_t endMonth) {
  _irregularity = 0;
  _beginDay = beginSunday;
  _beginMonth = beginMonth;
  _endDay = endSunday;
  _endMonth = endMonth;
}

flex_DST::flex_DST(uint8_t irregularity, uint8_t beginDay, uint8_t beginMonth,
...
uint8_t flex_DST::_daysInMonth(uint8_t month) {
  // DST never begins or ends in February so it is not considered
  if (month < 8) {
...
boolean flex_DST::checkDST(DateTime RTCTime) {
  ...
  if (currentMonth == _beginMonth) {
    ...
    if (_beginMonth < _endMonth) { // Northern Hemisphere
      return ((currentMonth > _beginMonth) && (currentMonth < _endMonth));
    } else {
      return ((currentMonth > _beginMonth) || (currentMonth < _endMonth));
    }
  }
